...
function Designmaster() {
  ...
  const fetchDesignData = async () => {
    try {
      ...
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: 'Failed to load design records',
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleDateFilter = (fromDate, toDate) => {
    if (fromDate || toDate) {
      const filtered = designData.filter((item) => {
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        if (response.ok) {
          Swal.fire('Deleted!', 'Design record has been deleted.', 'success');
          fetchDesignData();
        } else {
  ...
  const handleAddClick = () => {
    navigate('/designmasterform');
  };

  const handleEditClick = (record) => {
    navigate(`/designmasterform`, { state: { editingRecord: record } });
  };
